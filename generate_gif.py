...
def generate_gif(data, output_path):
    ...
    # Standing Table box
    # We will create a box in the upper-left of the chart for the current standings
    ...
    def update(frame):
        # Frame is 0-indexed. Since we want to pause at the end, the frame number can exceed num_gws - 1.
        # We cap the index to the last gameweek to freeze the chart.
        idx = min(frame, num_gws - 1)
        curr_gw = idx + 1
        
        # Sliced data for lines
        x_data = p1['events'][:curr_gw]
        y1_data = p1['points_history'][:curr_gw]
        y2_data = p2['points_history'][:curr_gw]
        y3_data = p3['points_history'][:curr_gw]
        ...
        # Update markers at current gameweek
        marker1.set_offsets([[curr_gw, y1_data[-1]]])
        marker2.set_offsets([[curr_gw, y2_data[-1]]])
        marker3.set_offsets([[curr_gw, y3_data[-1]]])
        
        # Update inline labels (offset slightly to the right)
        lbl1.set_position((curr_gw + 0.3, y1_data[-1]))
        lbl1.set_text(f"{y1_data[-1]} pts")
        
        lbl2.set_position((curr_gw + 0.3, y2_data[-1]))
        lbl2.set_text(f"{y2_data[-1]} pts")
        
        lbl3.set_position((curr_gw + 0.3, y3_data[-1]))
        lbl3.set_text(f"{y3_data[-1]} pts")
        
        # Update background GW text
        ...
        # Create standing table content (with padded strings for monospace alignment)
        ...
        for rank, (player_name, entry_name, pts, _) in enumerate(current_scores, 1):
            ...
            leaderboard_lines.append(f"{rank}. {disp_name:<22} {pts:>4} pts")
            
        table_text.set_text("\n".join(leaderboard_lines))
        
        # Adjust Y-limits dynamically as the lines grow to make it look smooth
    ...
    # Total frames: 37 gameweeks + 30 extra frames to freeze the final state
    total_frames = num_gws + 30
    ...
    # Save the animation
    ani.save(output_path, writer='pillow', fps=7)
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(output_dir, "fpl_standings_progression.gif")
    generate_gif(data, output_file)
